...
object StructuralIntegrationSpec extends ZIOSpecDefault {
  ...
  object Person {
    implicit val ts: ToStructural[Person] = ToStructural.derived[Person]
  }

  case class Company(name: String, ceo: Person)
  object Company {
    implicit val ts: ToStructural[Company] = ToStructural.derived[Company]
  }

  case class Group(members: List[Person])
  object Group {
    implicit val ts: ToStructural[Group] = ToStructural.derived[Group]
  }

  case class User(email: Option[String])
  object User {
    implicit val ts: ToStructural[User] = ToStructural.derived[User]
  }

  case class VectorGroup(items: Vector[Person])
  object VectorGroup {
    implicit val ts: ToStructural[VectorGroup] = ToStructural.derived[VectorGroup]
  }

  case class SetHolder(tags: Set[String])
  object SetHolder {
    implicit val ts: ToStructural[SetHolder] = ToStructural.derived[SetHolder]
  }

  case class MapHolder(data: Map[String, Int])
  object MapHolder {
    implicit val ts: ToStructural[MapHolder] = ToStructural.derived[MapHolder]
  }

  case class EitherHolder(result: Either[String, Int])
  object EitherHolder {
    implicit val ts: ToStructural[EitherHolder] = ToStructural.derived[EitherHolder]
  }
  ...
  def spec: Spec[TestEnvironment, Any] = suite("StructuralIntegrationSpec")(
    ...
    test("Collections in structural types") {
      ...
      assertTrue(members.map(_.selectDynamic("name")) == List("A", "B")) &&
      assertTrue(members.map(_.selectDynamic("age")) == List(1, 2))
    },
    ...
    test("structural type names include generic parameters (Scala 2 macro)") {
      case class Container(data: Map[String, List[Int]])
      implicit val ts: ToStructural[Container] = ToStructural.derived[Container]

      val tn = Schema.derived[Container].structural(ts).reflect.typeName.name

      assertTrue(tn == "{data:Map[String,List[Int]]}")
    },
